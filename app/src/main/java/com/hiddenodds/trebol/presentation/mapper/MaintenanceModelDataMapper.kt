...
class MaintenanceModelDataMapper @Inject constructor(val context: Context):
    ...
    override fun <E : RealmObject> transform(objectRealm: E?): IModel {
        ...
        val maintenance = objectRealm as Maintenance
    ...
    override fun <E : RealmObject> transform(objectRealmCollection: Collection<E>?):
            Collection<IModel> {
        val maintenanceCollection = objectRealmCollection!!
                .filterIsInstance<Maintenance>() as Collection<Maintenance>

        val maintenanceModelCollection: MutableCollection<IModel> = ArrayList()

        if (!maintenanceCollection.isEmpty())
